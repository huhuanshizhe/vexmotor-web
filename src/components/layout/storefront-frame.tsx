...
import { storefrontNavigationBase } from '@/lib/site-shell';
...
export async function StorefrontFrame({ title, description, eyebrow, actions, children }: StorefrontFrameProps) {
  ...
  const [homeData, navigation] = await Promise.all([
    getHomeData(),
    ...
    <main className="storefront-shell">
      ...
      <header className="storefront-header">
        <div className="storefront-header-main">
          ...
          <HeaderUtilityStrip links={navigation.utilityLinks} initialCartCount={0} preferences={preferences} />
        </div>
